...
Learn how to authenticate with the SecurePaste API.

## Authentication Methods

SecurePaste supports two authentication methods:
...
```bash
curl -X POST https://api.securepaste.com/auth/login \
...
```bash
curl https://api.securepaste.com/auth/me \
...
```bash
curl -X POST https://api.securepaste.com/auth/register \
  -H "Content-Type: application/json" \
  -d '{
    "email": "newuser@example.com",
...
```typescript
// Login
const response = await fetch('https://api.securepaste.com/auth/login', {
  method: 'POST',
  headers: {
...
// Use token
const userResponse = await fetch('https://api.securepaste.com/auth/me', {
  headers: {
...
response = requests.post(
    'https://api.securepaste.com/auth/login',
    json={
        'email': 'user@example.com',
        'password': 'password'
    }
...
user_response = requests.get(
    'https://api.securepaste.com/auth/me',
...
```bash
# Login
TOKEN=$(curl -X POST https://api.securepaste.com/auth/login \
  -H "Content-Type: application/json" \
