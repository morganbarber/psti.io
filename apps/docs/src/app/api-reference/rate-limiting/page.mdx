...
Understand psti.io's rate limiting policies and how to handle rate limits.

## Overview

psti.io implements rate limiting to prevent abuse and ensure fair usage for all users. Rate limits vary by endpoint and user type.
...
```typescript
const response = await fetch('https://api.psti.io.com/api/pastes');
...
async function getPaste(id: string) {
  ...
  const response = await fetch(`https://api.psti.io.com/api/pastes/${id}`);
...
await fetch('https://api.psti.io.com/api/pastes', {
  method: 'POST',
...
```typescript
class APIClient {
  private baseURL = 'https://api.psti.io.com';
...
class APIClient:
    def __init__(self, token: Optional[str] = None):
        self.base_url = 'https://api.psti.io.com'
        self.token = token
