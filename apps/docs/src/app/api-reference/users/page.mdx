...
```bash
curl https://api.psti.io.com/users/profile \
...
```bash
# Update email
curl -X PATCH https://api.psti.io.com/users/profile \
  -H "Authorization: Bearer YOUR_TOKEN" \
...
```bash
curl https://api.psti.io.com/users/stats \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X DELETE https://api.psti.io.com/users/account \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "password": "current-password",
...
```typescript
// Get profile
const profile = await fetch('https://api.psti.io.com/users/profile', {
  headers: {
...
// Update profile
await fetch('https://api.psti.io.com/users/profile', {
  method: 'PATCH',
  headers: {
    'Authorization': `Bearer ${token}`,
...
// Get stats
const stats = await fetch('https://api.psti.io.com/users/stats', {
  headers: {
    'Authorization': `Bearer ${token}`,
  },
});
...
response = requests.get(
    'https://api.psti.io.com/users/profile',
...
stats_response = requests.get(
    'https://api.psti.io.com/users/stats',
    headers={'Authorization': f'Bearer {token}'}
)
